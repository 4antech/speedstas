...
const conf = {
	     "az" : {"k1":17266, "k2":62529, "pinLeft": 27, "pinRight": 25, "enc": 1},
	     "el" : {"k1":33206, "k2":47333, "pinLeft":  0, "pinRight":  0, "enc": 2}
}
...
//	if (en==1) state.enc[en] = state.enc[en]+AZ_OFFSET; else state.enc[en] = state.enc[en]+EL_OFFSET;
	...
	if (((tms[en]-ltms[en])<40) && Math.abs(encs[en]-lencs[en])>250) {
...
pr = new Promise(function(resolv,reject){
	...
	ovect[en] = vect[en];
	if (state.enc[en]>state.last[en]) vect[en]=1; else vect[en]=0;
...
switch(en) {
 ...
 case 1:
	...
	if (((state.enc[1]<AZ_SOFTLIMIT_CW) && state.move[1]==0) || ((state.enc[1]>AZ_SOFTLIMIT_CCW) && state.move[1]==1)) {
	    port.write("2|1|0|z\n");
	    consolelog("SoftLimit! "+Math.abs(state.last[en]-state.enc[en])+" : "+state.enc[1]+"---");
	}

	Atomics.store(sharedArray,11,xtm);
	Atomics.store(sharedArray,10,(encs[en]));//	2 ENCODER  <<4 было
...
//	if (state.enc[2]<conf.el.k1 || state.enc[2]>conf.el.k2) {
	...
	if (((state.enc[2]>EL_SOFTLIMIT_DOWN) && state.move[2]==1) || ((state.enc[2]<EL_SOFTLIMIT_UP) && state.move[2]==0)) {
	    port.write("2|7|0|z\n");
	    consolelog("SoftLimit! "+Math.abs(state.last[en]-state.enc[en])+" : "+state.enc[2]+"---");
	}

	Atomics.store(sharedArray,14,xtm);
	Atomics.store(sharedArray,13,(encs[en]));
...
//    checkStop(en);
	...
	consolelog(">>"+en+" "+(state.enc[en]).toString(16)+" : "+state.last[en]+" grad:"+Math.round(state.enc[en]*36000/65535)/100+" speed:"+curspeed+" : "+(Date.now()-state.tm[en])+" : "+xspeed[en]);
...
consolelog("stop "+stop[v]+" : "+state.enc[v]);

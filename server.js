...
const azleft = new Gpio(27, {mode: Gpio.INPUT, pullUpDown: Gpio.PUD_UP, alert: true});
const azright = new Gpio(25, {mode: Gpio.INPUT, pullUpDown: Gpio.PUD_UP, alert: true});
...
azleft.on('alert', (level, tick) => {if (!level) xxstop('r');});
azright.on('alert', (level, tick) => {if (!level) xxstop('l');});
...
pr.then(()=>{
	...
	savedPos[en]  = enc[en];
	lgrad = grad;
	grad = Math.round(enc[en]*36000/65535)/100;
